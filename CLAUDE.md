...
- `Swatch.qml` — the overlay. Lifecycle contract with the shell: `open(payloadJson)`, `close()`, `opened` property. `shell`, `manifest`, `omarchyPath` are injected by the shell's Loader. `CacheImage` (inline component) is how every image is shown: a load that fails is retried on `cacheGen` ticks while `thumbs.sh` runs, declaratively, so bindings survive. Geometry goes through `root.sp()`/`root.fz.*`, frozen from `Style` at each `open()` — never `Style.space`/`Style.font` directly — so a candidate whose `shell.toml` sets `[spacing] scale` or font sizes re-lays out the real shell (faithful) but not the picker (see `freezeMetrics`). The filmstrip marks the live card with a **gate** — fixed corner brackets in the candidate's accent that the strip runs through — rather than a playhead line, which lands between two cards as readily as on one. Three things there are load-bearing: `stripArea` carries `sp(52)` of slack because `strip` clips and the live card lifts and scales past its own cell; the delegate wraps that card in a padded `Item` whose `layer.enabled` is `cell.selected`, so the shadow has somewhere to render and the strip holds one render target however many themes are in it; and the brackets sit on a keyline of the candidate's own background, because an accent can be the theme's foreground (`kanagawa`) or near-grey (`vantablack`) — position and depth do the marking, and the accent is the part allowed to be weak.
